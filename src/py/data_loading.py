"""
Load and normalize ExecuComp parquet tables for the Dittmann-Maug replication.

Handles column aliasing, type coercion, default filling, and validation for the
four required tables: anncomp, coperol, codirfin, and stgrttab.
...
def required_table_paths(config: RunConfig) -> dict[str, Path]:
    """Return a mapping from table name to expected parquet file path."""
    return {
...
def missing_required_tables(config: RunConfig) -> dict[str, Path]:
    """Return table names and paths for any required files that do not exist."""
...
def _apply_aliases(table_name: str, frame: pd.DataFrame) -> pd.DataFrame:
    """Rename columns using the table's alias map; skips aliases if target already exists."""
...
def _validate_required_columns(table_name: str, frame: pd.DataFrame) -> None:
    """Raise ValueError if any required columns are missing from the frame."""
...
def _apply_default_columns(table_name: str, frame: pd.DataFrame) -> pd.DataFrame:
    """Fill missing values (and add absent columns) using the table's default map."""
...
def _coerce_numeric_columns(table_name: str, frame: pd.DataFrame) -> pd.DataFrame:
    """Cast designated numeric columns to float, coercing unparseable values to NaN."""
    normalized = frame.copy()
...
def _coerce_int_columns(table_name: str, frame: pd.DataFrame) -> pd.DataFrame:
    """Cast designated columns to nullable Int64."""
    normalized = frame.copy()
...
def _normalize_table_values(table_name: str, frame: pd.DataFrame) -> pd.DataFrame:
    """Apply table-specific value normalization (uppercase flags, datetime parsing)."""
    normalized = frame.copy()
...
def _sort_table(table_name: str, frame: pd.DataFrame) -> pd.DataFrame:
    """Sort the frame by the table's canonical sort keys and reset the index."""
...
def normalize_table(table_name: str, frame: pd.DataFrame) -> pd.DataFrame:
    """Run all normalization steps for the given table and return the cleaned DataFrame."""
...
def _anncomp_lookup_for_coperol(anncomp: pd.DataFrame) -> pd.DataFrame:
    """Extract the deduplicated coperol-relevant columns from anncomp for use as a fallback."""
...
def _normalize_coperol_with_anncomp(raw_coperol: pd.DataFrame, anncomp: pd.DataFrame) -> pd.DataFrame:
    """Normalize coperol, filling missing execid/permid/ceoann from anncomp when possible."""
...
def load_required_tables(config: RunConfig) -> ExecuCompTables:
    """Read and normalize all four required parquet files; raise FileNotFoundError if any are missing."""
